...
export const AccountInfoSchema = lazySchema(() =>
  z
    .object({
      ...
      apiProvider: z
        .enum(['firstParty', 'bedrock', 'vertex', 'foundry', 'ollama'])
        .optional()
        .describe(
          'Active API backend. Anthropic OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, etc.). "ollama" uses local Ollama models.',
        ),
    })
