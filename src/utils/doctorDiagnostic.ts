...
> {
  ...
  const packagesToCheck = ['@sayantanmandal/claude-code']
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@sayantanmandal/claude-code') {
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @sayantanmandal/claude-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@sayantanmandal/claude-code'
        ) {
